...
- double-click `scripts/start_local_workspace_demo.bat` to bootstrap `ws_api_demo`, sync sample brief and personas, start the static server, restart the SaaS API and worker loop against the current repo code, wait for the authenticated session endpoint to respond, and open the hosted workspace page with the default demo token
