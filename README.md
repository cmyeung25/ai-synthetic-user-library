...
Run `node scripts\\verify_frontline_studio_smoke.mjs` to execute the browser smoke for `/studio`. The smoke starts an isolated temporary SaaS runtime, bootstraps a browser session, creates a study, uses the persona-library picker plus target-audience and moderator-guide fields, proposes a plan, confirms an approved plan, starts and processes a plan-linked research run, opens completed-run evidence review, saves and reloads an evidence view, creates and reloads a study report, creates a decision, creates and reloads a boundary-preserving share view, verifies drill-down nav-level exclusivity, route navigation, refresh persistence, browser back/forward, 1024px and 1280px horizontal overflow, critical panel overlap, and visible-copy guards, then writes artifacts under `output/playwright/frontline_studio_smoke/`.
